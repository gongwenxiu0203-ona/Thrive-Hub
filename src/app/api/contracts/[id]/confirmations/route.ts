...
export async function GET(_request: NextRequest, context: Context) {
  try {
    ...
    const rows = await prisma.contractProjectConfirmation.findMany({ where: { contractId: id }, orderBy: { createdAt: "asc" }, include: { versions: { orderBy: { version: "desc" }, select: { version: true, actorId: true, reason: true, createdAt: true } } } });
    const [bankAccounts, options, canEdit, canManage, users, confirmationTemplates, replacementAudits] = await Promise.all([
      ...
      prisma.financeAuditLog.findMany({ where: { entityType: "CONTRACT_CONFIRMATION", action: "ACTIVATE_REPLACEMENT", entityId: { in: rows.map(row => row.id) } }, select: { entityId: true } }),
    ]);
    const replacementCount = replacementAudits.reduce<Record<string, number>>((counts, row) => {
      counts[row.entityId] = (counts[row.entityId] ?? 0) + 1;
      return counts;
    }, {});
    return NextResponse.json({ contract, confirmations: rows.map(row => ({ ...decodeConfirmation(row), businessVersion: 1 + (replacementCount[row.id] ?? 0) })), bankAccounts, options, confirmationTemplates, canEdit, canManage, users, canRenumber: session.role === "ADMIN" && canManage }, { headers: { "Cache-Control": "private, no-store" } });
...
export async function POST(request: NextRequest, context: Context) {
  try {
    ...
    // 新增确认书必须以自身创建方式和自身签署文件为准，不能继承主合同原件。
    return NextResponse.json({ confirmation: decodeConfirmation(saved), activated: false }, { status: 201 });
  } catch (error) { return confirmationResponseError(error); }
}
