...
export interface DetailRecord {
  ...
  periods: {
    ...
    notes: string | null;
    channelReviewStatus:
      "DRAFT" | "PENDING" | "CONFIRMED" | "DISPUTED" | "SKIPPED";
...
const RECEIVED_CURRENCY_OPTIONS = [
  "USD",
  "CNY",
  "EUR",
  "GBP",
...
function ChannelPayeeCard({
  ...
  const [form, setForm] = useState<ChannelPayeeSnapshot>({
    ...EMPTY_PAYEE,
    ...payee,
  });
  ...
  async function save() {
    ...
    try {
      const response = await fetch(
        `/api/finance/channel-reconciliations/${reconciliationId}`,
        {
          method: "PATCH",
        ...
        <div className="mt-4 space-y-4 border-t border-slate-100 pt-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setField("paypalAccount", e.target.value)}
              />
            </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              disabled={saving || (hasSavedInfo && !reason.trim())}
              onClick={save}
            >
              {saving ? "保存中…" : "保存收款信息"}
            </button>
...
function RuleDrivenPeriodCard({
  ...
  const complete =
    (isFixed ? period.fixedFeeShareAmount : period.commissionShareAmount) !==
    null;
  ...
  const canPay =
    canEdit &&
    complete &&
    (reviewStatus === "CONFIRMED" || reviewStatus === "SKIPPED") &&
    period.financeReviewStatus === "APPROVED" &&
    !paidAt;
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const shareRate = isFixed
    ? period.fixedFeeShareRate
    : period.commissionShareRate;
  const shareAmount = isFixed
    ? period.fixedFeeShareAmount
    : period.commissionShareAmount;
  const paidAmount = isFixed
    ? period.fixedFeePaidAmount
    : period.commissionPaidAmount;
  ...
  async function uploadBusinessDocument() {
    if (!businessFile) return setError("请选择渠道发票、收据或结算凭证");
    setSaving(true);
    setError(null);
    try {
      const form = new FormData();
      form.append("file", businessFile);
      form.append("documentType", "CHANNEL_INVOICE");
      form.append("streamType", isFixed ? "FIXED_FEE" : "COMMISSION");
      const response = await fetch(
        `/api/finance/channel-reconciliations/${reconciliationId}/periods/${period.id}/business-documents`,
        { method: "POST", body: form },
      );
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) return setError(payload.error ?? "渠道凭证上传失败");
  ...
  async function reviewBusinessDocument(decision: "APPROVED" | "REJECTED") {
    if (!period.businessDocument) return;
    const reason =
      decision === "REJECTED" ? window.prompt("请填写驳回原因")?.trim() : "";
    if (decision === "REJECTED" && !reason) return;
    setSaving(true);
    setError(null);
    try {
      const response = await fetch(
        `/api/finance/channel-business-documents/${period.businessDocument.id}/review`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) return setError(payload.error ?? "渠道凭证审核失败");
      router.refresh();
    } finally {
      setSaving(false);
    }
  ...
  async function save() {
    ...
    if (!serviceStart || !serviceEnd || serviceEnd < serviceStart)
      return setError("请填写正确的服务周期起止时间");
    if (
      !isFixed &&
      ruleType === "A" &&
      ...
      return setError(
        `A 类佣金规则按 ${commissionRuleCurrency} 判断，请选择相同货币`,
      );
          ...
          body: JSON.stringify({
            ...(isFixed
              ? {
                  ...
                  ...(isFirstEntry
                    ? { fixedFeeReceivedCurrency: receivedCurrency }
                    : {}),
                }
              : {
                  commissionReceived: received === "" ? null : Number(received),
                  ...(isFirstEntry
                    ? { commissionReceivedCurrency: receivedCurrency }
                    : {}),
                  ...(ruleType === "B"
                    ? { confirmedGmv: gmv === "" ? null : Number(gmv) }
                    : {}),
                }),
  ...
  async function reviewFlow(action: "PUSH" | "SKIP") {
    ...
    try {
      const response = await fetch(
        `/api/finance/channel-reconciliations/${reconciliationId}/periods/${period.id}/review-flow`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            action,
            expectedVersion: period.channelReviewVersion,
          }),
        },
      );
      const payload = await response.json().catch(() => ({}));
  ...
  async function channelReview(action: "CONFIRM" | "DISPUTE") {
    if (action === "DISPUTE" && !disputeReason.trim())
      return setError("请填写异议原因");
    setSaving(true);
    setError(null);
    try {
      const response = await fetch(
        `/api/finance/channel-reconciliations/${reconciliationId}/periods/${period.id}/channel-review`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function uploadProofs(): Promise<string[] | null> {
    ...
    for (const file of proofFiles) {
      ...
      const response = await fetch(
        `/api/finance/channel-reconciliations/${reconciliationId}/periods/${period.id}/payment-proof`,
        { method: "POST", body: form },
      );
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
      ...
      if (typeof payload.paymentProofUrl === "string")
        uploaded.push(payload.paymentProofUrl);
    }
  ...
  async function savePayment() {
    if (
      proofFiles.length === 0 &&
      parsePaymentProofUrls(period.paymentProofUrl).length === 0
    ) {
      setError("请上传或粘贴付款回单");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `/api/finance/channel-reconciliations/${reconciliationId}/periods/${period.id}/payments`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            streamType: isFixed ? "FIXED_FEE" : "COMMISSION",
            amount: Math.max(0, (shareAmount ?? 0) - paidAmount),
            currency: displayCurrency,
            paidAt: paymentDate,
            proofUrls: paymentProofUrls,
            transactionNo: paymentNote.trim(),
          }),
        },
      );
      const payload = await response.json().catch(() => ({}));
  ...
  const statusMap = {
    DRAFT: ["待推送", "bg-slate-100 text-slate-600"],
    PENDING: ["待渠道商确认", "bg-amber-100 text-amber-700"],
    CONFIRMED: ["渠道商已确认", "bg-emerald-100 text-emerald-700"],
    DISPUTED: ["渠道商有异议", "bg-red-100 text-red-700"],
  ...
  return (
    <article
      className={`relative overflow-hidden rounded-xl border bg-white p-4 shadow-sm ${paidAt ? "border-emerald-200" : reviewStatus === "DISPUTED" ? "border-red-200" : "border-slate-200"}`}
    >
      <div className="absolute -left-[27px] top-5 h-3 w-3 rounded-full border-2 border-white bg-brand-500 ring-2 ring-brand-100" />
      {paidAt && (
        <div className="pointer-events-none absolute right-5 top-12 z-10 -rotate-12 rounded-md border-[3px] border-emerald-500/55 px-3 py-1 text-2xl font-black tracking-[0.18em] text-emerald-600/55">
          已付款
        </div>
      )}
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-slate-800">
            {period.periodLabel ?? `第 ${displayIndex} 期`}
          </p>
          <p className="mt-1 text-xs text-slate-400">
            第 {displayIndex} / {totalPeriods} 期 ·{" "}
            {paidAt ? "已付款并锁定" : complete ? "已录入" : "待录入"}
          </p>
        </div>
        <span
          className={`rounded-full px-2 py-1 text-xs font-medium ${status[1]}`}
        >
          {status[0]}
        </span>
      </div>
      <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-4 text-xs">
        <div>
          <dt className="text-slate-400">Thraive 实际到账</dt>
        ...
        <div>
          <dt className="text-slate-400">分账比例</dt>
          <dd className="mt-1 font-medium text-slate-700">
            {fmtPct(shareRate)}
          </dd>
        </div>
        <div>
          <dt className="text-slate-400">渠道商分账金额</dt>
          <dd className="mt-1 font-semibold text-emerald-600">
            {fmtMoney(shareAmount, displayCurrency)}
          </dd>
        </div>
        <div>
          <dt className="text-slate-400">向渠道商实际付款时间</dt>
          <dd className="mt-1 font-medium text-slate-700">
            {paidAt?.slice(0, 10) ?? "—"}
          </dd>
        </div>
        <div className="col-span-2">
          <dt className="text-slate-400">{cycleName}</dt>
          <dd className="mt-1 font-medium text-slate-700">{cycleLabel}</dd>
        </div>
      </dl>
      <div className="mt-3 grid grid-cols-3 gap-2 rounded-lg bg-slate-50 p-2 text-[11px]">
        <div>
          <p className="text-slate-400">渠道凭证</p>
          <p className="mt-1 font-medium text-slate-700">
            {period.businessDocumentStatus}
          </p>
        </div>
        <div>
          <p className="text-slate-400">财务审核</p>
          <p className="mt-1 font-medium text-slate-700">
            {period.financeReviewStatus}
          </p>
        </div>
        <div>
          <p className="text-slate-400">付款状态</p>
          <p className="mt-1 font-medium text-slate-700">
            {period.payableStatus}
          </p>
        </div>
        {period.businessDocument && (
          <div className="col-span-3 flex items-center justify-between border-t border-slate-200 pt-2">
            <a
              className="font-medium text-brand-600 hover:underline"
              href={period.businessDocument.fileUrl}
              target="_blank"
              rel="noreferrer"
            >
              查看渠道凭证
            </a>
            <span>本列已付 {fmtMoney(paidAmount, displayCurrency)}</span>
          </div>
        )}
        {period.businessDocument?.rejectionReason && (
          <p className="col-span-3 text-red-600">
            驳回原因：{period.businessDocument.rejectionReason}
          </p>
        )}
      </div>
      {["CONFIRMED", "SKIPPED"].includes(reviewStatus) &&
        !["PENDING", "APPROVED"].includes(period.businessDocumentStatus) && (
          <div className="mt-3 rounded-lg border border-dashed border-slate-200 p-3">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <input
                ...
                onChange={(event) =>
                  setBusinessFile(event.target.files?.[0] ?? null)
                }
              />
              <button
                type="button"
                className="btn-secondary text-xs"
                disabled={!businessFile || saving}
                onClick={() => void uploadBusinessDocument()}
              >
                提交财务审核
              </button>
            </div>
      ...
      {!isChannel &&
        canEdit &&
        period.businessDocument?.status === "PENDING" && (
          <div className="mt-3 flex justify-end gap-2">
            <button
              type="button"
              className="btn-secondary text-xs text-red-600"
              disabled={saving}
              onClick={() => void reviewBusinessDocument("REJECTED")}
            >
              驳回凭证
            </button>
            <button
              ...
              disabled={saving}
              onClick={() => void reviewBusinessDocument("APPROVED")}
            >
              审核通过
            </button>
          </div>
      ...
      {parsePaymentProofUrls(period.paymentProofUrl).map((url, index) => (
        <a
          key={`${url}-${index}`}
          href={url}
          target="_blank"
          rel="noreferrer"
          className="mr-3 mt-3 inline-flex items-center gap-1 text-xs font-medium text-brand-600 hover:underline"
        >
      ...
      {!isChannel &&
        canEdit &&
        ["PENDING", "CONFIRMED", "DISPUTED", "SKIPPED"].includes(
          reviewStatus,
        ) && (
      ...
      {!isChannel && canEdit && !paidAt && !reviewLocked && (
      ...
      {isChannel && reviewStatus === "PENDING" && !paidAt && (
        <div className="mt-4 space-y-3 border-t border-amber-100 pt-3">
          <p className="text-xs text-slate-600">
            请核对本期金额与服务周期。确认后数据将锁定；如有问题请填写具体原因。
          </p>
          <textarea
            className="input min-h-20 resize-y"
            value={disputeReason}
            onChange={(e) => setDisputeReason(e.target.value)}
            placeholder="选择有异议时，请填写异议原因"
          />
          {error && <p className="text-xs text-red-600">{error}</p>}
          <div className="flex justify-end gap-2">
            <button
              ...
              onClick={() => void channelReview("DISPUTE")}
            >
            ...
            <button
              ...
              disabled={saving}
              onClick={() => void channelReview("CONFIRM")}
            >
      ...
      {editing && !paidAt && !reviewLocked && !isChannel && (
            ...
            <div className="flex h-10 overflow-hidden rounded-lg border border-slate-200 bg-white focus-within:border-brand-400 focus-within:ring-2 focus-within:ring-brand-100">
              ...
              {isFirstEntry ? (
                <>
                  <input
                    aria-label="到账货币"
                    list="channel-currency-options"
                    maxLength={8}
                    className="w-20 border-l border-slate-200 bg-slate-50 px-2 text-sm font-medium uppercase text-slate-700 outline-none"
                    value={receivedCurrency}
                    onChange={(e) =>
                      setReceivedCurrency(e.target.value.toUpperCase())
                    }
                  />
                  <datalist id="channel-currency-options">
          ...
          {!isFirstEntry && (
            <div>
              <label className="label">
                {reviewStatus === "DISPUTED" ? "纠错原因" : "修改原因"} *
              </label>
              <input
                className="input"
                value={reason}
      ...
      {editingPayment && canPay && (
        <div className="mt-4 space-y-3 border-t border-emerald-100 bg-emerald-50/40 pt-3">
          ...
          <div>
            <label className="label">付款确认说明（选填）</label>
            <textarea
              className="input min-h-20 resize-y"
              value={paymentNote}
              ...
              <div className="mt-2 space-y-1">
                {proofFiles.map((file, index) => (
                  <div
                    ...
                    className="flex items-center justify-between gap-2 text-xs text-emerald-700"
                  >
      ...
      {error &&
        !editing &&
        !editingPayment &&
        !(isChannel && reviewStatus === "PENDING") && (
        ...
        <div className="rounded-xl border border-brand-200 bg-brand-50/40 p-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  setContractId(
                    nextCustomer?.contracts.length === 1
                      ? nextCustomer.contracts[0].id
                      : "",
                  );
                }}
              >
          ...
          <div className="mt-3 flex justify-end gap-2">
            ...
            <button
              ...
              disabled={saving}
            >
              {saving ? "保存中…" : "保存并重新生成周期"}
            </button>
          </div>
          <p className="mt-2 text-xs text-amber-700">
            已有金额、GMV 或付款记录时，系统会阻止重建周期。
          </p>
        </div>
      )}
      {!editing && error && (
...
function RuleDrivenDetail({
  ...
  const fixedPeriods = record.periods.filter(
    (period) => period.streamType !== "COMMISSION",
  );
  const commissionPeriods = record.periods.filter(
    (period) => period.streamType !== "FIXED_FEE",
  );
  const fixedCompleted = fixedPeriods.filter(
    (period) => period.fixedFeeShareAmount !== null,
  ).length;
  const commissionCompleted = commissionPeriods.filter(
    (period) => period.commissionShareAmount !== null,
  ).length;
  const streamTotal = (
    periods: DetailRecord["periods"],
    kind: StreamKind,
  ...
  const fixedTotal = streamTotal(
    fixedPeriods,
    "fixed",
    record.fixedFeeReceivedCurrency,
  );
  const commissionTotal = streamTotal(
    commissionPeriods,
    "commission",
    record.commissionReceivedCurrency,
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-slate-900">
            {record.customer.brandName} · 渠道商分账
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            合同 {record.contract?.contractNo ?? "—"} · 渠道商{" "}
            {record.channelUser.name}
          </p>
        </div>{" "}
        {!isAdmin && !isChannel && (
          <div className="flex gap-2">
            <button
              ...
              className="btn-secondary"
              onClick={() =>
                window.alert("只有管理员可以修改主记录，请联系管理员")
              }
            >
              修改主记录
            </button>
            <button
              ...
              onClick={() =>
                window.alert("只有管理员可以删除主记录，请联系管理员")
              }
            >
              删除主记录
            </button>
      ...
      {isAdmin && (
        <AdminRecordActions
          record={record}
          customerOptions={adminCustomerOptions}
        />
      )}
      <ChannelPayeeCard
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="card p-4">
          <p className="text-xs text-slate-400">固费总期数</p>
          <p className="mt-1 text-2xl font-semibold">{fixedPeriods.length}</p>
          <p className="text-xs text-emerald-600">已录入 {fixedCompleted} 期</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-slate-400">固费分账累计</p>
          <p className="mt-1 text-2xl font-semibold text-emerald-600">
            {fixedTotal}
          </p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-slate-400">抽佣总期数</p>
          <p className="mt-1 text-2xl font-semibold">
            {commissionPeriods.length}
          </p>
          <p className="text-xs text-emerald-600">
            已录入 {commissionCompleted} 期
          </p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-slate-400">抽佣分账累计</p>
          <p className="mt-1 text-2xl font-semibold text-emerald-600">
            {commissionTotal}
          </p>
        </div>
        ...
        {(["fixed", "commission"] as const).map((kind) => (
          <section key={kind} className="card p-5">
            ...
            <div className="ml-3 space-y-4 border-l-2 border-slate-100 pl-5">
              {(kind === "fixed" ? fixedPeriods : commissionPeriods).map(
                (period, index, periods) => (
                  <RuleDrivenPeriodCard
                    ...
                    commissionRuleCurrency={
                      record.splitRule?.commissionThresholdCurrency ?? "USD"
                    }
                    currency={
                      kind === "fixed"
                        ? record.fixedFeeReceivedCurrency
...
function buildWaterfall(derived: PeriodDerived[]): WaterfallEvent[] {
  ...
  for (const p of derived) {
    ...
    if (p.confirmedFee !== null || p.confirmedCommission !== null) {
      ...
      const proxyAt =
        p.feeReceivedAt ??
        p.commissionReceivedAt ??
        `${p.monthLabel}-01T00:00:00Z`;
      events.push({
        at: proxyAt,
        kind: "confirmed",
...
export function ChannelReconciliationDetail({
  ...
  const [legacyDeleteError, setLegacyDeleteError] = useState<string | null>(
    null,
  );
  ...
  const summary = derivedPeriods.reduce(
    (acc, p) => {
      if (p.feeReceivedAt && p.channelReceivableFee !== null)
        acc.feeTotal += p.channelReceivableFee;
      if (p.commissionReceivedAt && p.channelReceivableCommission !== null)
        acc.commissionTotal += p.channelReceivableCommission;
      return acc;
    },
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="card p-5">
          <div className="flex items-center gap-2 text-xs text-slate-400">
            <DollarSign className="h-4 w-4" /> 固费分账总额（按 Thraive
            实际收款汇总）
          </div>
          ...
          <p className="mt-1 text-[11px] text-slate-400">
            分账比例{" "}
            {fmtPct(record.splitRule?.fixedFeeRate ?? record.fixedFeeShareRate)}
          </p>
        </div>
        <div className="card p-5">
          <div className="flex items-center gap-2 text-xs text-slate-400">
            <TrendingUp className="h-4 w-4" /> 佣金分账总额（按 Thraive
            实际收款汇总）
          </div>
          <p className="mt-2 text-3xl font-bold text-emerald-600">
            {fmtMoney(summary.commissionTotal)}
          </p>
          <p className="mt-1 text-[11px] text-slate-400">
          ...
          <div className="grid gap-3 text-sm sm:grid-cols-3">
            <div>
              ...
              <p className="font-medium text-slate-800">
                {record.splitRule.ruleType === "A" ? "A 基础" : "B 阶梯"}
              </p>
            </div>
            <div>
              <p className="text-[11px] text-slate-400">截止日期</p>
              <p className="font-medium text-slate-800">
                {formatDate(new Date(record.splitRule.splitEndDate))}
              </p>
            </div>
            <div>
              <p className="text-[11px] text-slate-400">固费分账比例</p>
              <p className="font-medium text-slate-800">
                {fmtPct(record.splitRule.fixedFeeRate)}
              </p>
            </div>
          </div>
                  ...
                  {tieredRules.map((t, i) => (
                    <tr key={i} className="border-t border-slate-100">
                      ...
                      <td className="px-3 py-1.5">
                        {t.gmvMax === null ? "+∞" : fmtMoney(t.gmvMax)}
                      </td>
                ...
                {derivedPeriods.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-2 text-right font-medium text-slate-800">
                      {fmtMoney(p.confirmedFee)}
                    </td>
                    <td className="px-2 py-2 text-right text-slate-500">
                      {fmtPct(p.fixedFeeRate)}
                    </td>
                    <td className="px-2 py-2 text-right font-semibold text-emerald-600">
                      {fmtMoney(p.channelReceivableFee)}
                    </td>
                    <td className="px-2 py-2 text-right text-slate-600">
                      {fmtMoney(p.confirmedGmv)}
                    </td>
                    <td className="px-2 py-2 text-right font-medium text-slate-800">
                      {fmtMoney(p.confirmedCommission)}
                    </td>
                    <td className="px-2 py-2 text-right text-slate-500">
                      {fmtPct(p.channelCommissionRate)}
                    </td>
                    <td className="px-2 py-2 text-right font-semibold text-emerald-600">
                      {fmtMoney(p.channelReceivableCommission)}
                    </td>
                    <td className="px-2 py-2 text-slate-600">
                      {p.dueDate ? (
      ...
      <div className="card p-5">
        <p className="mb-4 text-sm font-semibold text-slate-700">
          时间流瀑布（仅显示对账确认 & Thraive 实际收款）
        </p>
        {waterfall.length === 0 ? (
          <p className="text-xs text-slate-400">
            暂无事件 — 等客户对账确认或 Thraive 实际收款后此处会自动填充
          </p>
        ) : (
            ...
            {waterfall.map((e, i) => {
              ...
              return (
                <li key={i} className="relative">
                  <div
                    className={`absolute -left-[34px] flex h-7 w-7 items-center justify-center rounded-full border-2 ${cfg.color}`}
                  >
                  ...
                  <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span
                            className={`rounded-full border px-2 py-0.5 text-[10px] font-medium ${cfg.color}`}
                          >
                            {cfg.label}
                          </span>
                        ...
                        {e.dueDate && (
                          <p className="mt-1 inline-flex items-center gap-1 rounded bg-blue-50 px-1.5 py-0.5 text-[11px] text-blue-700">
                            <Calendar className="h-3 w-3" /> 渠道商付款截止：
                            {formatDate(new Date(e.dueDate))}（实收+7工作日）
                          </p>
                        )}
                      </div>
