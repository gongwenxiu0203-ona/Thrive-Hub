...
type Row = { id: string; number: string; title: string; status: string; version: number; signedFileUrl: string | null; draft: Draft; versions: { version?: number; versionNo?: number; createdAt?: string }[] };
type Data = {
...
const states: Record<string, string> = { DRAFT: "草稿", EFFECTIVE: "已生效", ACTIVE: "已生效", TERMINATED: "已终止", VOID: "已作废" };
...
function initial(contractId: string): Draft {
  return { contractId, title: "", brand: "", storeUrl: "", startDate: null, endDate: null, minimumMonths: 6,
...
export function ConfirmationEditor({ contractId }: { contractId: string }) {
  ...
  const editable = !!data?.canEdit && (!editing || editing.status === "DRAFT");
  const unsaved = !!editing && !!draft && JSON.stringify(editing.draft) !== JSON.stringify(draft);
  function open(row: Row | null) {
    if (draft && !window.confirm("离开当前编辑内容？未保存的修改会丢失。")) return;
    const next = row ? structuredClone(row.draft) : initial(contractId);
    if (!row && data) {
    ...
    setEditing(row); setDraft(next); setReason(""); setFile(null); setError(""); setNotice(""); setNewNumber(row?.number || ""); setNumberReason("");
  }
  ...
  async function save() {
    ...
    try {
      ...
      const body = await request(editing ? `${base}/${editing.id}` : base, { method: editing ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editing ? { draft, expectedVersion: editing.version, reason } : { draft }) });
      const fresh = await load(); const saved = fresh.confirmations.find(r => r.id === body.confirmation.id);
      if (saved) { setEditing(saved); setDraft(structuredClone(saved.draft)); } setReason(""); setNotice(previouslySigned && !saved?.signedFileUrl ? "草稿已保存。由于确认书内容发生修改，原盖章版仅保留在历史版本中；请重新上传与当前内容一致的签署原件。" : "草稿已保存。上传签署原件后，可按生效条件继续处理。");
  ...
  async function upload() {
    ...
    setBusy(true); setError(""); setNotice("");
    try { const form = new FormData(); form.set("file", file); form.set("expectedVersion", String(editing.version)); form.set("reason", reason); await request(`${base}/${editing.id}`, { method: "PUT", body: form }); const fresh = await load(); const saved = fresh.confirmations.find(r => r.id === editing.id); if (saved) setEditing(saved); setFile(null); setNotice("签署原件已存档；系统不会自动识别或覆盖表单字段。"); } catch (e) { setError(e instanceof Error ? e.message : "上传失败"); } finally { setBusy(false); }
  }
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-10">
    ...
    <header className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-semibold text-slate-900">项目确认书</h1><p className="mt-2 text-sm text-slate-600">{data?.contract.contractNo} · {data?.contract.customer?.brandName} · 同一主合同下，每份生效确认书独立计费、独立对账。</p></div>{data?.canEdit && <Button variant="primary" disabled={busy} onClick={() => open(null)}>新建项目确认书</Button>}</header>
    {data?.contract.contractMode === "FRAMEWORK" && <div className="flex flex-wrap items-center gap-3 rounded-lg border border-purple-100 bg-white p-4"><label className="flex min-w-0 items-center gap-2 text-sm">导出范围<select className={control} value={exportSelection} onChange={e => setExportSelection(e.target.value)}><option value="both">主格式合同＋当前项目确认书</option><option value="master">仅主格式合同</option><option value="confirmation">仅当前项目确认书</option></select></label><Button disabled={busy || (exportSelection !== "master" && !editing)} onClick={exportDocument}>导出已填合同（Word）</Button><span className="text-xs text-slate-500">先选择并保存确认书；签署区域保留供双方签署。</span></div>}
    ...
    {draft && data && <section className="rounded-xl border border-purple-100 bg-white p-4 sm:p-6">
      ...
      <fieldset disabled={!editable || busy} className="min-w-0 space-y-8 disabled:opacity-80">
        ...
        {editable && <Button variant="primary" loading={busy} onClick={save}>保存草稿</Button>}
      </fieldset>
