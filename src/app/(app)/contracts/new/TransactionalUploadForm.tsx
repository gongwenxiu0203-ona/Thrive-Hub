...
export function TransactionalUploadForm({
  ...
  function submit(formElement: HTMLFormElement) {
    ...
    const submittedOwnerId = String(fd.get("ownerId") ?? "").trim();
    const submittedStartDate = String(fd.get("startDate") ?? "").trim();
    const submittedEndDate = String(fd.get("endDate") ?? "").trim();
    const submittedFile = fd.get("file");
    if (!submittedOwnerId) {
      setError("请选择合同负责人");
      return;
    }
    ...
    if (!submittedStartDate || !submittedEndDate) return setError("请填写合同开始时间和截止时间");
    if (submittedStartDate > submittedEndDate) return setError("合同截止时间不能早于开始时间");
    startTransition(async () => {
      fd.set("type", "TRANSACTIONAL");
  ...
  return (
    <form className="card space-y-5 p-6" onSubmit={(event) => { event.preventDefault(); submit(event.currentTarget); }}>
      <input type="hidden" name="ownerId" value={ownerId} />
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">合同开始时间 *</label>
          <input name="startDate" className="input" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">合同截止时间 *</label>
          <input name="endDate" className="input" type="date" min={startDate || undefined} value={endDate} onChange={(e) => setEndDate(e.target.value)} />
        </div>
      </div>
