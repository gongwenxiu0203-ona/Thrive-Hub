...
export type RecycleType = "customer" | "contract" | "affiliate" | "task" | "reminder" | "salesBatch" | "salesRecord" | "project" | "workLog";
...
export async function purgeExpired(): Promise<void> {
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (prisma.salesRecord.deleteMany as any)({ where });
    ...
    // 工作日志
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (prisma.workLog.deleteMany as any)({ where });
  } catch (e) {
